...
export default function CashierHistoryPage() {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      const params: { date_from?: string; date_to?: string; payment_status?: string } = {
        date_from,
        date_to,
      };

      if (selectedPaymentMethod !== "all") {
        params.payment_status = selectedPaymentMethod === "cash" ? "paid" : selectedPaymentMethod;
      }

      const response = await fetchCompletedOrders(params);
      ...
      setAlertModal({
        ...
        message: "Tidak dapat memuat riwayat pesanan. Silakan coba lagi nanti.",
        variant: "error",
      });
    } finally {
      setIsLoading(false);
    }
  }, [periodFilter, selectedPaymentMethod]);
      ...
      <div className="mx-auto max-w-7xl px-3 sm:px-4 py-4 sm:py-6 lg:py-8">
        ...
        <div className="mb-4 sm:mb-6 lg:mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900">Riwayat Pesanan</h1>
          </div>
          <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-slate-600">
            Lihat dan cetak ulang invoice untuk pesanan yang sudah selesai
          </p>
        </div>

        {/* Period Filter */}
        <div className="mb-4 sm:mb-5 lg:mb-6 flex flex-wrap gap-2">
          {(["today", "yesterday", "this_week", "this_month"] as PeriodFilter[]).map((period) => (
            <button
              key={period}
            ...
            <div className="sm:w-48">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none rounded-xl border border-slate-200 bg-white px-3 py-2.5 pr-8 text-sm font-medium text-slate-700 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                >
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-xs text-slate-500">
                {searchQuery
                  ...
                  : "Belum ada pesanan selesai pada periode yang dipilih"}
              </p>
            </div>
          ) : (
            <>
